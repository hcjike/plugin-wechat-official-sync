...
| `40007` | invalid media_id | 媒体 id 不合法 | 封面 `thumb_media_id` 无效/过期、属于其它公众号或是临时素材。**建草稿时收到它无需手动处理**：插件会作废该封面的缓存记录、重新上传一张并重试一次（只重试一次） |
