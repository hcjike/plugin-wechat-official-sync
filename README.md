...
转存正文图片（解析 HTML → uploadimg → 替换为微信图片地址）
        │
        ▼
美化正文排版（按标签注入微信友好的内联样式，代码块重建为微信原生结构、宽表格支持横向滚动，安全清理 script / 事件属性）
        │
        ▼
