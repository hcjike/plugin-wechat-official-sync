...
- 插件（或 Halo 服务）重启时，未完成的同步任务会被**自动恢复**：启动后按持久化的任务输入自动重放一遍完整同步流程（封面与正文图片会重新转存），期间状态继续显示「同步中」（悬停可看到「正在自动恢复」）。注意：恢复是「用缓存输入重新执行」而非断点续传——若中断恰好发生在草稿创建成功之后、状态写回之前，重放可能多出一份草稿，按需在公众号后台删除即可。单个任务最多执行 3 次（首次提交 + 中断后的自动恢复），仍未能完成时标记为失败，需手动重新同步。
