 ...
 * 对解析到的每一个 IP 再次调用 {@link SsrfGuard#isBlockedAddress(InetAddress)}，命中受限网段即让
 * 解析失败，从而使连接无法建立，堵住预检与连接之间的时间差（TOCTOU）。</p>
 *
...
final class SsrfSafeAddressResolverGroup extends AddressResolverGroup<InetSocketAddress> {
    ...
    protected AddressResolver<InetSocketAddress> newResolver(EventExecutor executor) {
        NameResolver<InetAddress> filtered =
            new SsrfSafeNameResolver(executor, new DefaultNameResolver(executor));
